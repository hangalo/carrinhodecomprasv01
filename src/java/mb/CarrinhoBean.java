...
public class CarrinhoBean implements Serializable {
    ...
    private int isExisting(Produto p) {
        for (int i = 0; i < this.carrinho.size(); i++) {
            if (this.carrinho.get(i).getIdProduto().getIdProduto() == p.getIdProduto()) {
                return i;
            }
        }
        return -1;
    ...
    public long suma() {
        ...
        for (Item it : this.carrinho) {
            s += it.getQuantidade() * it.getIdProduto().getPrecoProduto();
        }
        return s;
    }
    ...
    public void registarCompra() {
        ...
        Integer numeroFatura = facturaFacade.lastFactura();
